...
/**
 * Channel logger — writes JSON lines to a daily log file:
 *   <stateDir>/logs/wechat-YYYY-MM-DD.log
 *
 * (Vendored from OpenClaw; repointed off the openclaw tmp dir to the EchoAI
 * channel state dir, since this runs as a standalone EchoAI channel process.)
 */

const MAIN_LOG_DIR = path.join(resolveStateDir(), "logs");
const SUBSYSTEM = "echo-wechat";
const RUNTIME = "node";
...
function resolveMainLogPath(): string {
  ...
  return path.join(MAIN_LOG_DIR, `wechat-${dateKey}.log`);
}
