...
interface PackageJson {
  ...
  ilink_appid?: string;
  /**
   * iLink client version used for the protocol header, independent of the npm
   * package `version`. Lets the EchoAI package keep its own version while
   * sending the protocol-meaningful client version inherited from upstream.
   */
...
/** Protocol client version (falls back to the npm package version). */
const ILINK_CLIENT_VERSION = pkg.ilink_client_version ?? pkg.version ?? "0.0.0";

const CHANNEL_VERSION = ILINK_CLIENT_VERSION;
...
const ILINK_APP_CLIENT_VERSION: number = buildClientVersion(ILINK_CLIENT_VERSION);
...
export async function apiGetFetch(params: {
  ...
  const controller =
    (timeoutMs != null && timeoutMs > 0) || params.abortSignal ? new AbortController() : undefined;
  const t =
