...
async function pollQRStatus(apiBaseUrl: string, qrcode: string, verifyCode?: string, abortSignal?: AbortSignal): Promise<StatusResponse> {
...
async function refreshQRCode(
  ...
  try {
    ...
    if (verbose) process.stdout.write(`🔄 二维码已更新，请重新扫描。\n\n`);
    if (onQrCode) {
      await onQrCode(qrResponse.qrcode_img_content);
    } else {
...
export async function waitForWeixinLogin(opts: {
  ...
  while (Date.now() < deadline) {
    ...
    try {
      ...
      const statusResponse = await pollQRStatus(currentBaseUrl, activeLogin.qrcode, activeLogin.pendingVerifyCode, opts.abortSignal);
      if (opts.abortSignal?.aborted) {
        activeLogins.delete(opts.sessionKey);
        return { connected: false, message: "登录已取消。" };
      }
      ...
      switch (statusResponse.status) {
        ...
        case "need_verifycode": {
          ...
          const verifyPrompt = activeLogin.pendingVerifyCode
            ? "❌ 你输入的数字不匹配，请重新输入："
            : "输入手机微信显示的数字，以继续连接：";
          const code = await (opts.readVerifyCode ?? readVerifyCodeFromStdin)(verifyPrompt);
